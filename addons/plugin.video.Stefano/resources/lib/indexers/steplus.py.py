...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'http://www.stefanoaddon.info/test/home4.xml'
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.Stefano_list, link))
    ...
    def Stefano_list(self, url, result=None):
        try:
...
def setxxxpass():
	d = xbmcgui.Dialog().yesno('[COLOR red]Stefano[/COLOR]','Would You Like To Set a Password for the XXX Section?')
	if d:
		...
		if (kb.isConfirmed()):
			...
			if pw =="":
				...
				xbmcaddon.Addon('script.module.steplus').setSetting('xxxpass',pw)
				xbmcaddon.Addon('script.module.steplus').setSetting('enablexxxpass','true')
