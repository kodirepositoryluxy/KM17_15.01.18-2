...
ADDON=xbmcaddon.Addon(id='plugin.video.KodiMod')
...
def CATEGORIES():
    link = OPEN_URL('https://pastebin.com/raw/LedFaia6').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("Kodi Mod","In Download ",'', 'Attendere Prego')
    ...
    time.sleep(2)
    dp.update(0,"", "Sto estraendo i file, attendi un attimo")
    print '======================================='
    print addonfolder
    print '======================================='
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("DOWNLOAD COMPLETATO", 'Per vedere le modifiche della nuova Build occorre riavviare Kodi', 'Clicca su Ok e su Close,')
    killxbmc()
        
      
        
def killxbmc():
    choice = xbmcgui.Dialog().yesno('Force Close Kodi', 'Chiusura Forzata di Kodi', 'Vuoi continuare?', nolabel='No, Cancel',yeslabel='Yes, Close')
    if choice == 0:
        return
    elif choice == 1:
        pass
