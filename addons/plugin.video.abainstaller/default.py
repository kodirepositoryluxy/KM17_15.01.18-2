...
ADDON=xbmcaddon.Addon(id='plugin.video.abainstaller')
...
def CATEGORIES():
    link = OPEN_URL('http://pastebin.com/raw/AZ9RLF4J').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("Add-On","In Download ",'', 'Attendere Prego')
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("DOWNLOAD COMPLETATO", 'Per utilizzare il nuovo add-on', 'Clicca su Ok poi su SI, Chiudi,')
    killxbmc()
        
      
        
def killxbmc():
    choice = xbmcgui.Dialog().yesno('Force Close Kodi', 'Chiusura Forzata di Kodi', 'Vuoi continuare?', nolabel='No, Cancel',yeslabel='SI, Chiudi')
    if choice == 0:
        return
    elif choice == 1:
        pass
