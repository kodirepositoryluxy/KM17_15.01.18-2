...
if xbmcaddon.Addon('plugin.video.kodilivetv').getSetting('download_path') == "":
    dst = os.path.join( os.path.join(xbmc.translatePath("special://userdata/addon_data" ), 'plugin.video.kodilivetv'), 'download', 'tmp')
else:
    dst = os.path.join(xbmcaddon.Addon('plugin.video.kodilivetv').getSetting('download_path'), 'tmp')
