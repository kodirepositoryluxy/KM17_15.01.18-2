...
class DownloadFile(object):
    ...
    def __init__(self, url, localFileName=None, auth=None, timeout=120.0, autoretry=True, retries=500, origurl=None, res = None):
        ...
        self.url = common.urlresolve(url).split("|")[0]
    ...
    def __downloadFile__(self, urlObj, fileObj, callBack=None, origurl = None, res = None):
        ...
        if self.cur < 1:
            ...
            xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%('Download file: ', Namefile, 4100, icon))
            xbmc.log("####### Downloader Start Download ########", xbmc.LOGNOTICE)
            xbmc.log(Namefile + " downloading...", xbmc.LOGNOTICE)
        ...
        while 1:
            ...
            if data == "":
                ...
                if self.retries > self.curretry:
                    self.curretry += 1
                    self.resume()
                    #xbmc.log("Download auto resume : " + str(self.curretry), xbmc.LOGNOTICE)
                else:
                    break
            else:                    
                fileObj.write(data)
            ...
            if int(self.fileSize) <= int(self.cur):
                ...
                if self.stopme == False:
                    xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(Namefile, localizedString(10209).encode('utf-8'), 3500, icon))
                    self.curretry = 501
                    self.timeout=1.0
                    self.stopme = True
                    xbmc.log("####### Downloader Finished Download ########", xbmc.LOGNOTICE)                   
    ...
    def __retry__(self):
        ...
        if self.retries > self.curretry:
                self.curretry += 1
                xbmc.log("Download auto resume - retry : " + str(self.curretry), xbmc.LOGNOTICE)
                if self.getLocalFileSize() != self.urlFilesize:
                    self.resume()
        else:
            xbmc.log('Retries all used up', xbmc.LOGNOTICE)
    ...
    def __startHttpResume__(self, restart=None, callBack=None):
        ...
        if self.auth:
            self.__authHttp__()
        try:    
            req = urllib2.Request(self.url, headers={ 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:51.0) Gecko/20100101 Firefox/51.0' })
    ...
    def getUrlFileSize(self):
        ...
        if self.type == 'http' or self.type == 'https':
            ...
            try:
                ...
                request = urllib2.Request(self.url, headers={ 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:51.0) Gecko/20100101 Firefox/51.0' })
    ...
    def download(self, callBack=None):
        ...
        if self.auth:
            ...
            try:
                ...
                #req = urllib2.Request(self.url, headers={ 'Referer' : domain, 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0' })
                
                req = urllib2.Request(self.url, headers={ 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:51.0) Gecko/20100101 Firefox/51.0' })
                urllib2Obj = urllib2.urlopen(req, timeout=self.timeout)
                self.__downloadFile__(urllib2Obj, f, callBack=callBack)
                
                #xbmc.log("Download: " + str(self.url), xbmc.LOGNOTICE)
                #xbmc.log("Referer: " + domain, xbmc.LOGNOTICE)
                ...
                xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(Namefile, "DOWNLOAD ERROR: failed connection to server!", 4800, icon))
