...
def OpenURL(url, headers={}, string=""):
    
    pw = False
    if re.search("https?:\/\/\w+:\w*@",url):
        
        urlnopass = url.split("nopass/")[0]
        datafile =  os.path.join( pwdir , base64.standard_b64encode(urlnopass))
        URL1 = url.split("@")[0]
        ...
        if os.path.isfile(datafile):
            ...
            if us == "x" and string == "":
                stringa = GetKeyboardText("Enter username", "")
                if len(stringa) < 1:
                    return
                us = stringa
            if pw == "x" and string == "":
                stringa = GetKeyboardText("Enter password", "")
                if len(stringa) < 1:
                    return
                pw = stringa
    ...
    #xbmc.log("OpenURL - URL--> " + url,xbmc.LOGNOTICE)
    ...
    for k, v in headers.items():
            req.add_header(k, v)
            xbmc.log("OpenURL --> req.add_header('"+k+"', '"+v+"')",xbmc.LOGNOTICE)
    
    from urllib2 import URLError, HTTPError                
    try:                
        response = urllib2.urlopen(req, timeout=20)       
    except URLError as e:
        xbmc.log("KLTV OpenURL function - We failed to reach a server. Reason : " + e.reason, xbmc.LOGERROR)
        return ""
        pass
    except HTTPError as e:
        xbmc.log("KLTV OpenURL function - The server couldn't fulfill the request.. Error code : " + e.code, xbmc.LOGERROR)
        return ""
        pass
...
def cachelist(url,cdir,tcache=82800):
    ...
    if check_url(url):
        ...
        if os.path.isfile(tmp) and t < tcache:
            ...
            try:
                ...
                content = OpenURL(surl,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64; Trident/7.0; rv:11.0) like Gecko'})
            except:
                content = OpenURL(url,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64; Trident/7.0; rv:11.0) like Gecko'})
...
def pornHD(url):
    ...
    return urlvideo.group(1).replace("\/","/")
...
def find_lpanel(url):
    ...
    if url.find("get.php?username=")>0:
        ...
        #xbmc.log("----:" + str(len(list)), xbmc.LOGNOTICE)
...
def StartDowloader(url,name,mode,DFolder):
    ...
    if len(ext)<2 or len(ext)>3 and not ext == "part": 
        ext = "mp4" 
    ...
    file = dpath + name + '.' + ext
    #xbmc.log("Download URL --> " + str(url), xbmc.LOGNOTICE)
    #xbmc.log("Download file --> " + str(file), xbmc.LOGNOTICE)
...
def cachepage( url, s, headers={} ):
    ...
    if t > s or not os.path.isfile(tmp):
        ...
        if len(content) > 0 :
            write_file(tmp, content)
            xbmc.sleep(300)
            #xbmc.log('Write temp file : ' + tmp + ' - size : ' + format( len(content) ), xbmc.LOGNOTICE )    
    else:
        content = ReadFile(tmp)
        
    return content

def get_media_url_streammoe(url):

    if url.find("embed")>0:
        url = url.replace("https://","")
        URI = url.split('/')
        url = "https://" + URI[0] + "/" + URI[2]
    data = OpenURL(url,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64; Trident/7.0; rv:11.0) like Gecko'}) 
    pattern = "atob\('([^',]+)"
    atob = base64.decodestring(re.search(pattern,data).group(1))
    pattern = 'source src="([^",]+)'
    url = re.search(pattern,atob).group(1)
    #xbmc.log("****** download url" + url, xbmc.LOGNOTICE)
    return url

def get_media_url_akvideo(url):
    data = OpenURL(url,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64; Trident/7.0; rv:11.0) like Gecko'}) 
    cdn = re.search('\|fastcdn\|([^\|,]+)',data).group(1)
    token = re.search('\|480p\|([^\|,]+)',data).group(1)
    url = "https://" + cdn + ".fastcdn.stream/" + token + "/v.mp4"
    #xbmc.log("****** download url =" + url, xbmc.LOGNOTICE)
    return url

def get_media_url_vivosx(url):
    data = OpenURL(url,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64; Trident/7.0; rv:11.0) like Gecko'}) 
    r = re.search(r'Core\.InitializeStream \(\'(.*?)\'\)', data)
    b = base64.b64decode(r.group(1))
    j = json.loads(b)
    return j[0]

def get_media_url_rapidcom(url):
    data = OpenURL(url,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64; Trident/7.0; rv:11.0) like Gecko','Referer':url}) 
    pattern = 'source src="([^",]+)'
    url = re.search(pattern,data).group(1)
    #xbmc.log("****** download url" + url, xbmc.LOGNOTICE)
    return url    

def get_media_url_jwp(url):
    data = OpenURL(url,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64; Trident/7.0; rv:11.0) like Gecko','Referer':url}) 
    pattern = 'src="([^",]+)'
    url1 = re.search(pattern,data).group(1)
    
    data = urllib2.urlopen(url1)
    for line in data.readlines():
        xbmc.log("line = " + line, xbmc.LOGNOTICE)
        if re.match("(.*)AUTH_(.*)", line):
            url = re.search('"file" *: *"([^",]+)',line).group(1)
            xbmc.log("line = " + line, xbmc.LOGNOTICE)
            return url
...
def urlresolve(url):
    if url.find("openload.")>0:
        ...
        video_urls = openload.get_video_url(url)
        url = video_urls[0][1]        
    elif url.find("stream.moe")>0:
        url = get_media_url_streammoe(url)
    elif url.find("akvideo.")>0:
        url = get_media_url_akvideo(url)        
    elif url.find("fastvideo.")>0:
        from servers import fastvideo
        video_urls = fastvideo.get_video_url(url)
        url = video_urls[0][1]
    elif url.find("rapidvideo.com")>0:
        url = get_media_url_rapidcom(url)
    elif url.find("rapidvideo.")>0:
        from servers import rapidvideo
        video_urls = rapidvideo.get_video_url(url)        
        url = video_urls[0][1]
    elif url.find("streamango.")>0:
        import urlr
        url = urlr.resolve(url)         
        #from servers import streamango
        #video_urls = streamango.get_video_url(url)        
        #url = video_urls[0][1]
    elif url.find("speedvideo.")>0:
        from servers import speedvideo
        if not url.find("embed")>0:
            url = url.replace("speedvideo.net/","speedvideo.net/embed-")        
            if not url.find(".html")>0:
                url = url + ".html"
        video_urls = speedvideo.get_video_url(url)        
        url = video_urls[0][1]
    elif url.find("thevideo.me")>0:
        from servers import thevideome
        video_urls = thevideome.get_video_url(url)  
        url = video_urls[0][1]
    elif url.find("wstream.")>0:
        from servers import wstream
        video_urls = wstream.get_video_url(url)         
        url = video_urls[0][1]
    elif url.find("nowvideo.")>0:
        from servers import nowvideo
        url = url.replace("5nowvideo.com","nowvideo.li")
        url = url.replace("nowvideo.sx","nowvideo.li")
        video_urls = nowvideo.get_video_url(url)         
        url = video_urls[0][1]
    elif url.find("megadrive.")>0:
        from servers import megadrive
        video_urls = megadrive.get_video_url(url)         
        url = video_urls[0][1]
    elif url.find("megahd.")>0:
        from servers import megahd
        video_urls = megahd.get_video_url(url)         
        url = video_urls[0][1]         
    elif url.find("vidto.me")>0:
        from servers import vidtome
        if not url.find("embed")>0:
            url = url.replace("vidto.me/","vidto.me/embed-")
        video_urls = vidtome.get_video_url(url)         
        url = video_urls[0][1]
    elif url.find("vivo.sx")>0:    
        url = get_media_url_vivosx(url)
    elif url.find("jwplatform.com")>0:    
        url = get_media_url_jwp(url)
    #xbmc.log("*** Video url = " + url, xbmc.LOGNOTICE)    
    return url
