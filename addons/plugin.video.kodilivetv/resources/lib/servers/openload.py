...
def get_video_url(page_url, premium=False, user="", password="", video_password=""):
    ...
    try:
        ...
        n1, n2 = scrapertools.find_multiple_matches(data, "parseInt\('([^']+)',8\)")
        n3, n4 = scrapertools.find_single_match(data, "\-(\d+)\+0x4\)/\((\d+)\-0x8\)\)")
        ...
        for encode in var_encodes:
            ...
            try:
                ...
                while i < len(rango1):
                    ...
                    value4 ^= ((int(n1, 8) - int(n3) + 4) / (int(n4) - 8)) ^ int(n2, 8)
                    value5 = index1 * 2 + 127
                    for h in range(4):
