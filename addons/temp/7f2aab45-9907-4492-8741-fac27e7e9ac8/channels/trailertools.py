...
def buscartrailer(item, trailers=[]):
    ...
    # List of actions if executed from the context menu
    if item.action == "manual_search" and item.contextual:
        ...
        # Unselect Search Trailer from context menu to avoid redundancy
...
def abandomoviez_search(item):
    ...
    patron = '(?:<td width="85"|<div class="col-md-2 col-sm-2 col-xs-3">).*?<img src="([^"]+)"' \
             '.*?href="([^"]+)">(.*?)(?:<\/td>|<\/small>)'
    matches = scrapertools.find_multiple_matches(data, patron)
    # If there is only one result, search the trailers directly, but list all the results
    if len(matches) == 1:
