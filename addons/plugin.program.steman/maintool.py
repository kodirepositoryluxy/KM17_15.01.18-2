...
def purge_packages():
    ...
    if confirm:
        try:
            ...
            dialog = xbmcgui.Dialog()
            dialog.ok(AddonTitle, "Error Deleting Packages please visit https://t.me/steaddon forums")
    ...
    if confirm:
        ...
        try:
            for root, dirs, files in os.walk(addonpath,topdown=False):
                print root
                if root != addonpath :
                    if 'plugin.program.steman' not in root:
...
def AUTO_CLEAN_ON_OFF():
    ...
    HOME         =  xbmc.translatePath('special://userdata/addon_data/plugin.program.steman')
