...
uploaderpath=xbmc.translatePath(os.path.join('special://home','addons','script.steman.debug.log'))
...
def main_menu():
    ...
    if int(kodi.get_setting('notify')) == 1:
        xbmcgui.Dialog().notification('Serve  Aiuto?','https://t.me/steaddon',artwork+'icon.png',3000,False)
    ...
    kodi.addDir("Manutenzione Tools",'','call_maintool',artwork+'maintool.png',description="Keep your Kodi setup running at optimum performance!")
    ...
    kodi.addDir("Informazioni Sistema",'','system_info',artwork+'system_info.png',description="Useful information about your Kodi setup!")
    ...
    if kodi.get_setting('notifications-on-startup') == 'false':
        kodi.addItem("Notifications (Opt Out)",'','enable_notify',artwork+'notification_optout.png',description="Unsubscribe from important Stefano notifications!")
    if kodi.get_setting('notifications-on-startup') == 'true':
        kodi.addItem("Notifications (Opt In)",'','disable_notify',artwork+'notification_in.png',description="Subscribe to important Stefano notifications!")

    viewsetter.set_view("sets")

#################
def do_log_uploader():
    xbmc.executebuiltin("RunAddon(script.steman.debug.log)")
...
def rtmp_lib():
    liblist = "https://stefanoaddon.info/steman/rtmplist.txt"
...
def fullspeedtest():
    ...
    SpeedTest = base64.b64decode("aHR0cHM6Ly9zdGVmYW5vYWRkb24uaW5mby9zdGVtYW4vc3BlZWR0ZXN0ZmlsZS50eHQ=")
...
######MAIN TOOL
    ...
    if _is_debugging() == True:
        choice = xbmcgui.Dialog().yesno('steman', 'Please confirm that you wish to disable log debugging mode immediately')
        if choice == 1:
            xbmc.executebuiltin("ToggleDebug")
            xbmc.executebuiltin("Container.Refresh")
        else: quit()
    else:
        choice = xbmcgui.Dialog().yesno('steman', 'Please confirm that you wish to enable log debugging mode immediately')
        if choice == 1:
            xbmc.executebuiltin("ToggleDebug")
            xbmc.executebuiltin("Container.Refresh")
        else: quit()
    ...
    choice = xbmcgui.Dialog().yesno('steman', 'Please confirm that you wish to reload the skin cache immediately.')
    if choice == 1:
        xbmc.executebuiltin("ReloadSkin()")
    else: quit()
    
elif mode== 'updateaddons':
    choice = xbmcgui.Dialog().yesno('steman', 'Please confirm that you wish to force update all addons and repositories immediately.')
    if choice == 1:
    ...
    choice = xbmcgui.Dialog().yesno('steman', 'Would you like to hide or unhide your passwords?',yeslabel='Hide',nolabel='Unhide')
