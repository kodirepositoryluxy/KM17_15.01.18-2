...
def CAT():
	...
	addDir('Area Adulti','URL',31,icon,fanart,'')
	...
	if xbmcaddon.Addon().getSetting('enablexxxpass')=='true':
		...
		if (kb.isConfirmed()):
			...
			if pw == xbmcaddon.Addon().getSetting('xxxpass'):
				...
				xbmcgui.Dialog().ok('[COLOR red][B]Stefano Addon Area Adulti Sezione[/B][/COLOR]','Incorrect Password, Please Try Again')
				return
	else:
		addDir("The Best Videos",xxxurl+'/best',24,icon,fanart,'')
		addDir("Latest Videos",xxxurl,24,icon,fanart,'')
...
def setxxxpass():
	d = xbmcgui.Dialog().yesno('[COLOR red]Stefano Addon Area Adulti[/COLOR]','Imposta la Password Per Area Adulti ')
	if d:
		...
		if (kb.isConfirmed()):
			pw = kb.getText()
			if pw =="":
				xbmcgui.Dialog().notification('[COLOR red]Password Cannot Be Blank[/COLOR]','Stefano Addon')
				setxxxpass()
			else:
				xbmcaddon.Addon().setSetting('xxxpass',pw)
				xbmcaddon.Addon().setSetting('enablexxxpass','true')
				xbmcgui.Dialog().ok('[COLOR red]Stefano Addon[/COLOR]','Password has been set')
...
def xxxgenre(url):
		if xbmcaddon.Addon().getSetting('enablexxxpass')=='true':
			kb = xbmc.Keyboard ('', 'Inserisci XXX Password', False)
			kb.doModal()
			if (kb.isConfirmed()):
				pw = kb.getText()
				if pw == xbmcaddon.Addon().getSetting('xxxpass'):
					...
					xbmcgui.Dialog().ok('Attenzione','Password incorretta, Please Try Again')
					return
			else:
				xbmcgui.Dialog().ok('Attenzione','Password incorretta, Please Try Again')
				return
...
def showpremiumimage():
	premium_jpg = xbmc.translatePath(os.path.join('special://home/addons/script.module.steplus/resources/premium', 'premium_image.jpg'))
...
# OpenELEQ: query & type-parameter (added to line above)
	...
	xbmc.executebuiltin('XBMC.RunScript(script.module.steplus)')
	...
	xbmc.executebuiltin('Addon.OpenSettings(plugin.video.Stefano)')
	sys.exit()
